...
export class Tetris extends React.Component<TetrisProp, TetrisState> {
  ...
  private CreateDefaultBoard(): string[][] {
    const rows: string[][] = [];
  ...
  private initialState: TetrisState = {
  ...
  public constructor(props: TetrisProp) {
    super(props);
  ...
  private MoveInDirection(x: number, y: number): void {
  ...
  private RotateClockwise(tetramino: Tetramino): Tetramino {
  ...
  private keyPressHandle = (event: KeyboardEvent) => {
    if (
      this.state == null ||
  ...
  public GetLevelFromStatePanel = (stage: number): void => {
